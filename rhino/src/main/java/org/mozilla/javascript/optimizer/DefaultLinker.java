...
class DefaultLinker implements GuardingDynamicLinker {
    ...
    private GuardedInvocation getNameInvocation(
        ...
        } else if (op.isOperation(RhinoOperation.PREINCREMENT)) {
            mh = makeNameIncrement(lookup, name, 0);
        } else if (op.isOperation(RhinoOperation.PREDECREMENT)) {
            mh = makeNameIncrement(lookup, name, Node.DECR_FLAG);
        } else if (op.isOperation(RhinoOperation.POSTINCREMENT)) {
            mh = makeNameIncrement(lookup, name, Node.POST_FLAG);
        } else if (op.isOperation(RhinoOperation.POSTDECREMENT)) {
            mh = makeNameIncrement(lookup, name, Node.POST_FLAG | Node.DECR_FLAG);
        }
    ...
    private MethodHandle makeNameIncrement(MethodHandles.Lookup lookup, String name, int op)
            throws NoSuchMethodException, IllegalAccessException {
        MethodType tt =
                MethodType.methodType(
                        Object.class, Scriptable.class, Context.class, String.class, Integer.TYPE);
        MethodHandle mh = lookup.findStatic(DefaultLinker.class, "nameIncrDecr", tt);
        mh = MethodHandles.insertArguments(mh, 2, name, op);
        return mh;
    }

    // It's a lot to reorder the arguments to the existing method here, so create
    // a wrapper.
    @SuppressWarnings("unused")
    private static Object nameIncrDecr(Scriptable scope, Context cx, String name, int op) {
        return ScriptRuntime.nameIncrDecr(scope, name, cx, op);
    }
    ...
    private GuardedInvocation getMathInvocation(
        ...
        } else if (op.isOperation(RhinoOperation.COMPAREGT)) {
            mh = makeCompare(lookup, Token.GT);
        } else if (op.isOperation(RhinoOperation.COMPARELT)) {
            mh = makeCompare(lookup, Token.LT);
        } else if (op.isOperation(RhinoOperation.COMPAREGE)) {
            mh = makeCompare(lookup, Token.GE);
        } else if (op.isOperation(RhinoOperation.COMPARELE)) {
